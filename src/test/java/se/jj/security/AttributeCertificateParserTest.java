package se.jj.security;


import org.junit.jupiter.api.Test;
import static org.junit.jupiter.api.Assertions.*;


public class AttributeCertificateParserTest {

    private static final String AC_BASE64 = "";

    @Test
    public void testParse() throws Exception {
        AttributeCertificateParser parser = new AttributeCertificateParser();
        AttributeCertificateParser.ACInfo info = parser.parse(AC_BASE64);

        assertNotNull(info.holderSerial());
        assertNotNull(info.notBefore());
        assertNotNull(info.notAfter());
    }
}